...
parser.add_argument('-hex','--hexidecimal', dest='hexformat', action='store_true',
                           help='output file is hexidecimal (rather than py)')
...
#   0xff,0xff,0xff,0xff,0xff,
    ...
    if args.hexformat:
        for key, value in font.iteritems():
            print("0x%02x, %d,%d," % (key, len(value[0]), len(value)))
            for r in value:
                for c in r:
                    print("0x%02x," %(c)),
                print("")
